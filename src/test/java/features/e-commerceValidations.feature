Feature: Validating Ecommerce API's
  Scenario Outline: Verify if order is created
    ...
    And Message "message" in body is "Product Added Successfully"
    Then user create order
    And "Order Placed Successfully" is the message "message" in the body response
    Then user delete product ordered
    And  "Product Deleted Successfully" is the message "message" displayed
